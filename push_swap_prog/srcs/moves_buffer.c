...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/07/21 07:29:44 by moguy             #+#    #+#             */
/*   Updated: 2019/07/22 00:20:53 by moguy            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int							moves_buffer(char *move, int flush)
{
	...
	static unsigned int		siz = 1024;
	static unsigned int		index = 0;

	if (!buff)
	{
		if (!(buff = (int*)malloc(sizeof(int) * siz)))
			return (-1);
		ft_memset(buff, 0, sizeof(int) * siz);
	}
	if (flush == 1)
	{
	...
	else
	{
		if ((buff[index++] = parse_instruction(move)) == -1)
			return (-1);
		if (index == siz && (siz *= 2) && !(buff = realloc_buff(buff, siz)))
			return (-1);
	}
	return (0);
}
