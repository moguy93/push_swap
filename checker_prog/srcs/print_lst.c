...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/07/21 07:17:18 by moguy             #+#    #+#             */
/*   Updated: 2019/07/27 16:56:18 by moguy            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				pad(int size, bool free_mem)
{
	...
	if (!pad_buff)
	{
		if (!(pad_buff = ft_strnew((size_t)size)))
			return ;
		ft_memset(pad_buff, ' ', sizeof(char) * (unsigned long)size);
	}
